...
# Trees at once. Therefore, this is not advised.
# ------------------------------------------------------------------
...
def differentiate(originalDag, arg_name, variable_ranks):
    diffDag = None
    ...
    if arg == None: # Argument is not in expression
        diffDag = Tree(NODETYPE.CONSTANT, f'0_{Tree.new_constant()}')
        diffDag.rank = originalRank * 2
        diffDag.axes = originalDag.axes + originalDag.axes
        return originalDag, diffDag
    diffDag = Tree(NODETYPE.DELTA, f'delta_{Tree.new_delta()}')   # Derivative of the top node y with respect to itself
    diffDag.rank = originalRank * 2
    diffDag.axes = originalDag.axes + originalDag.axes
...
def _split_adj(dag):
    if dag.type == NODETYPE.SPECIAL_FUNCTION and dag.name == 'adj':
        dag = Tree(NODETYPE.PRODUCT, '*(,ij->ij)', Tree(NODETYPE.SPECIAL_FUNCTION, 'det', None, dag.right), Tree(NODETYPE.SPECIAL_FUNCTION, 'inv', None, dag.right))
...
def _contributions(node, diff, arg, yRank):
    ...
    if node in _originalNodeToDiffNode:   # If we've been to this node before, we need to add the new contribution to the old one
        diff = Tree(NODETYPE.SUM, '+', _originalNodeToDiffNode[node], diff)
...
if __name__ == '__main__':
    ...
    d = differentiate(originalDag, arg_name, variable_ranks)
    print(d)
    print_axes_help(d)
