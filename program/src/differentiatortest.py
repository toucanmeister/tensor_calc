...
class DifferentiatorTests(unittest.TestCase):
    ...
    def test_arctan(self):
        ...
        d = Differentiator(test)
        d.differentiate()
        self.assertEqual(str(d.diffDag), '(_delta(2) *(ba,a->ba) (elementwise_inverse(((x *(a,a->a) x) + 1))))')
    ...
    def test_sign(self):
        ...
        d = Differentiator(test)
        d.differentiate()
        self.assertEqual(str(d.diffDag), '(_delta(2) *(ba,a->ba) 0)')
    ...
    def test_adj(self):
        ...
        d = Differentiator(test)
        d.differentiate()
        self.assertEqual(str(d.diffDag), '(((_delta(4) *(abij,ij->ab) (inv(X))) *(cd,ab->cdab) ((adj(X)) *(ij,->ji) 1)) + ((_delta(4) *(abij,->abij) (det(X))) *(efcd,cdab->efab) ((-((inv(X)))) *(ij,kl->ijkl) (inv(X)))))')
