...
While automatic symbolic differentiation of matrix expressions and numerical differentiation of tensor expressions are well researched, automatic symbolic differentiation of tensor expressions is less so.
In this thesis, we design and implement a tensor calculus that allows for automatic symbolic differentiation of tensor expressions of any order while using an Einstein-like product notation that is more general than the notation used in the popular Ricci calculus.
...
We take a large part of the syntax for tensor multiplication from the established Python software package \textit{Numpy}, specifically its \codeword{einsum} procedure.
...
As an example of tensor products, see the inner, outer and elementwise product of two order 1 tensors (vectors) $x$ and $y$: $x*_{(i,i \rightarrow)}y$, $x*_{(i,j \rightarrow ij)}y$, $x*_{(i,i \rightarrow i)}y$.
...
But in the expressio $\text{det}(X + 1)$, $1$ would need to be an order 2 tensor filled with ones, because it is summed with another order 2 tensor.
...
\begin{table}[ht]
    ...
    \begin{tabular}{l | l | l}
        ...
        \codeword{det} & - &$()$ \\
    \end{tabular}
    \caption{Rules applied when propagating axis downward through a DAG.}
...
The procedure starts with the topmost node $y$, representing the entire expression, and computes its pullback, which is the derivative with respect to itself.
...
Proofs for the correctness of these rules can be found in the paper \cite{tensorpaper} by Laue, Mitterreiter and Giesen.
...
If $u$ is a product node representing the subexpression $v *_{(s_1,s_2 \rightarrow s_3)} w$ with index strings $s_1$, $s_2$ and $s_3$ in a tensor expression where the topmost node has indices $s_4$, then we may apply the product rule as follows.
...
%\FloatBarrier
%\section{DAG Growth}
%Each differentiation rule only adds a constant number of nodes.
%The only other added nodes during differentiation are when multiple paths to the same node are found.
%Each parent edge after the first one adds a sum node.
%Maximal so viele Knoten wie im nicht-vereinfachten binären Baum waren?
...
Continuing from the topmost node, we compute its contribution to the pullback of the left child first.
...
\subsection{Problem}
We discovered that the differentiation rule for product nodes sometimes creates invalid product nodes.
Specifically in the case where an index appears only in the index string of one input of a product node, but not in the output index string.
...
The root of this problem can be found in the proof of the reverse mode differentiation product rule in the paper \cite{tensorpaper} by Laue, Mitterreiter and Giesen (Theorem 8).
...
To find the derivative, the authors of the paper start from the definition of the Fr\'{e}chet Derivative applied to the product node and transform it to describe the pullback of $B$.
In the end, we wish to have an expression that ends with a form of $x \circ h$, because that makes $x$ the Fr\'{e}chet Derivative we are looking for.
Here, $\circ$ describes an inner product.
In our notation, an inner product has the form $x *_{(s_1 s_2, s_2 \rightarrow s_1)} h$ for two index strings $s_1$ and $s_2$.
...
Consider the paper's definition of the associative property for tensor products.
$$
...
\subsection{Solution}
To solve this problem, we forego the paper's version of the associative property and find another way to transform $\bar{C} *_{(s_4s_3,s_3 \rightarrow s_4)} (A *_{(s_1,s_2 \rightarrow s_3)} h)$ into an inner product.
...
$$
Next, we write the inner sum as in our Einstein-like tensor product notation.
...
The last two simplification steps are possible because the output indices $s_4$ is always be chosen to be disjoint to $s_1$, $s_2$ and $s_3$.
...
$$
We know that the output indices of this expression need to be $s_4$.
Using this, we can rewrite the outer sum as in our tensor product notation as well.
$$
...
For any product node of the form $A *_{s_1, s_{21} s_{22} \rightarrow s_3} B$ where $s_{21} \cap (s_1 s_3) = s_{21}$ and $s_{22} \cap (s_1 s_3) = \emptyset$, we call $s_{22}$ the missing indices of this node and transform by making the sum over the missing indices explicit.
...
In our example from the start, we would transform the original expression from $A *_{(ij,j \rightarrow )} v$ to $(A *_{(ij,j \rightarrow i)} v) *_{(i,i \rightarrow)} 1$.
...
Furthermore, the our algorithms could be analyzed and optimized in regards to efficiency and performance for different inputs.
