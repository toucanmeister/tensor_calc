...
In this case, $u$ adds nothing of its own to the pullback of $v$.
Thus, when differentiating a sum node like the one seen in figure \ref{fig:sum_rule}, the pullback of both $A$ and $X$ is simply the pullback of the sum node.
That is, $\frac{dy}{d(A+X)}$ is the same as $\frac{dy}{dA}$ and $\frac{dy}{dX}$.
...
If $u$ is a product node representing the subexpression $v *_{(s1, s2, s3)} w$ in a tensor expression with output indices \codeword{s4}, then we may apply the product rule as follows.
The contribution of $u$ to the pullback of $v$ is \codeword{}
