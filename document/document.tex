...
A formal characterization of the product $C = A *_{(s_1,s_2 \rightarrow s_3)} B$ is given in Equation \ref{eq:product}.

\begin{equation}
    C[s_3] = \sum_{(s_1 \cup s_2)  \backslash s_3} A[s_1] \cdot B[s_2]
    \label{eq:product}
\end{equation}
...
\subsection{Problem and Programmatic Solution}
In the proof of the reverse mode differentiation product rule in the paper \cite{tensorpaper} by Laue, Mitterreiter and Giesen (Theorem 8) in the fourth equality, the associative property for tensor products is used to transform $\bar{C} *_{(s_4s_3,s_3 \rightarrow s_4)} (A *_{(s_1,s_2 \rightarrow s_3)} h)$ into $(\bar{C} *_{(s_4s_3,s_1 \rightarrow s_4s_2)} A) *_{(s_4s_2,s_2 \rightarrow s_4)} h$. 
However, as can be seen by comparing the equality with the corresponding Lemma 1 on the previous page of the paper, the associative property would require that $s_1 = s_3 s_2$, which is not always the case.
...
In this section, we prove that the way we addressed the problem in the previous chapter, making the sum over the missing axes explicit, actually solves the problem.
Consider the left side of the fourth equality in the paper \cite{tensorpaper} by Laue, Mitterreiter and Giesen.
$$
\bar{C} *_{(s_4s_3,s_3 \rightarrow s_4)} (A *_{(s_1,s_2 \rightarrow s_3)} h)
$$
...
$$
In the end, we wish to have an expression that ends with a form of $x \circ h$, because that makes $x$ the derivative we are looking for.
To accomplish this, we split off part of the large sum to create two smaller sums over different indices.
$$
...
= s_1 s_3 s_4 \setminus (s_1 s_3 \cap \bar{s_2})
= s_1 s_3 s_4 \cap \bar{(s_1 s_3 \cap \bar{s_2})}
= s_1 s_3 s_4 \cap (\bar{s_1 s_3} \cup s_2)
= (s_1 s_3 s_4 \cap \bar{s_1 s_3}) \cup (s_1 s_3 s_4 \cap s_2)
= (s_1 s_3 s_4 \setminus s_1 s_3) \cup (s_1 s_3 s_4 \cap s_2)
= (s_4 \setminus s_1 s_3) \cup (s_1 s_3 s_4 \cap s_2) %TODO: FINISH THIS UND FÜGE IMPLIZITE ANNAHMEN HINZU
$$
