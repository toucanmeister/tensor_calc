...
Additionally, this calculus will use Einstein summation notation to represent tensor products, rather than the more complex notation used in Ricci calculus. % Todo: Why?
...
To differentiate a tensor expression, three kinds of information are needed: 1. The tensor expression itself, 2. The variable with respect to which should be differentiated, 3. The tensor rank of all variables in the expression.
...
The main point of interest is how to represent tensors and their products.
...
It differentiates between the two forms by the location of their indices, either in sub- or superscript. % Maybe elaborate here, bring some examples
While Ricci calculus is elegant in the way it handles the duality of tensors, the number of indices make it less readable.
...
Since tensors may be multiplied in many ways depending on their order, we need a way to describe which tensor axes should be multiplied in what way. % Maybe some examples here
...
If an index appears in multiple inputs, the respective axes will be multiplied.
...
As an example, two order-1 tensors (vectors) \codeword{x} and \codeword{y} could have their outer product, \codeword{x*(i,j->ij)y}, their inner product, \codeword{x*(i,j->)y} or their elementwise product, \codeword{x*(i,i->i)y}, taken.
The application of a matrix \codeword{A} to a vector \codeword{v} would be written as \codeword{A*(ij,j->ij)v}, while the elementwise product of two matrices \codeword{A} and \codeword{B} would be \codeword{A*(ij,ij->ij)B}.
