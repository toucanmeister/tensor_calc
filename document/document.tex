...
In dieser Arbeit wird ein Kalkül für die automatische symbolische Ableitung von Tensorausdrücken jeglichen Ranges erstellt und implementiert.
...
In this thesis, we design and implement a calculus for automatic symbolic differentiation of tensor expressions of any order.
...
\begin{figure}
    ...
    \begin{tikzpicture}[main/.style={draw, circle, minimum size=30pt, line width=2pt}, info/.style={rectangle, rounded corners=5, draw}, n/.style={-latex, line width=0.75pt}, d/.style={-latex, line width=0.75pt, dashed}]
        ...
        \draw [->, n] (sin) -- (+);
    \end{tikzpicture}
    \caption[Example of Binary Tree Generated from an Expression]{Binary expression tree generated from the expression string \texttt{A + sin(b)}. Empty nodes are omitted.}
...
\begin{table}[ht]
    ...
    \end{tabular}
    \caption[Subtree Substitutions Applied During Preprocessing]{Table showing all operations that get substituted during preprocessing, along with their substitutes. The transformation of $A^B$ is only applied when both $A$ and $B$ contain the differentiation argument, and is only valid when no entries of $A$ are zero. $(.^*)$ is a placeholder for elementwise multiplication; it is replaced with the appropriate tensor product during tensor order determination. $\text{elementwise\_inverse}$ is an elementwise function for which a differentiation rule is implemented.}
...
\begin{figure}
    ...
    \begin{minipage}{7cm}
        ...
        \end{tikzpicture}}
    \end{minipage}
    \caption[Example of Common Subtree Elimination Performed on an Expression DAG]{\textit{Left}: Binary tree generated representing the expression $(X+1) + X *_{(ij,ij \rightarrow ij)} (X+1)$. \textit{Right}: DAG obtained by eliminating common subtrees in the left tree.}
...
\begin{table}[ht]
    ...
    \begin{tabular}{l | l}
        ...
        \codeword{det} & $()$ \\
    \end{tabular}
    \caption[Rules for Detemining Axes of A node Depending on its Type]{Rules applied to determine axes of a node $X$, depending on the type of node it is.}
...
\begin{figure}
    ...
    \caption[Simplified Differentiation DAG in Example of Multiple Pullback Contributions]{Full simplified differentiation DAG of the original DAG in Figure \ref{fig:alg_example_1}.}
