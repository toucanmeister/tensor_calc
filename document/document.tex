...
\title{
\includegraphics[scale=0.7]{logo.png} \vspace{60pt}

Designing and Implementing a Tensor Calculus \vspace{30pt}
...
In the Study Program 'Computer Science' \vspace{20pt}

FRIEDRICH SCHILLER UNIVERSITY JENA 

Faculty of Mathematics and Computer Science \vspace{40pt}
...
\chapter*{Zusammenfassung} %TODO: In die Gruppe schicken
Für Optimierungsprobleme spielen Ableitungen eine zentrale Rolle bei der Findung einer optimalen Lösung.
Das automatische symbolische Ableiten von Matrixausdrücken und das numerische Ableiten von Tensorausdrücken sind gut erforscht und werden in der Industrie intensiv genutzt.
Das Feld der automatischen symbolischen Ableitung von Tensorausdrücken ist hingegen weniger gut untersucht.
In dieser Arbeit wird ein Tensorkalkül für die automatische symbolische Ableitung von Tensorausdrücken jeglichen Ranges erstellt und implementiert.
...
Weiterhin ermöglicht die Darstellung der Tensorausdrücke als gerichtete azyklische Graphen es, das Wachstum der Ausdrücke durch die Ableitung als linear in der Knoten- und Kantenzahl des Graphen abzuschätzen. 
...
Furthermore, its representation of tensor expressions as directed acyclic graphs allows us to bound the growth of the expression induced by differentiation to be linear in the number of edges and nodes in the graph.
...
The grammar, presented here in extended backus-naur form, is based on a standard grammar for mathematical expressions by Julien Klaus. %Todo: Source here? NO
...
\begin{figure}
    ...
    \begin{minipage}{7cm}
        ...
        \end{tikzpicture}}
    \end{minipage}
    \caption{\textit{Left}: Binary tree generated representing the expression $(X+1) + X *_{(ij,ij \rightarrow ij)} (X+1)$. \textit{Right}: DAG obtained by eliminating common subtrees in the left tree.}
...
\begin{figure}
    ...
    \end{minipage}
    \caption{\textit{Left}: A product node representing a scalar-vector-product in an expression DAG. "..." represents the rest of the DAG. \textit{Right}: Derivative of the left DAG with respect to $x$ computed using the product rule. "..." represents the pullback of the product node. Note that the product indices in the right DAG depend on the indices $s_4$ of the root node in the left DAG (not visible here), which is assumed to be scalar in this case.}
    \label{fig:prod_rule}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Progression of the differentiation procedure on the DAG in Figure \ref{fig:diff_example_original}. \textit{Left}: Pullback of the root node. \textit{Middle}: One contribution to the pullback of $X$. \textit{Right}: Pullback of $X$, both contributions summed.}
...
\begin{figure}
    ...
    \caption{Progression of the differentiation procedure on the DAG in Figure \ref{fig:alg_example_1}. \textit{Left}: Pullback of the $+$, $\text{sin}$ and $\text{cos}$ nodes. \textit{Middle}: First contribution to the pullback of $v$. \textit{Right}: Full pullback of $v$.}
...
\begin{figure}
    ...
    \end{minipage}
    \caption{\textit{Left}: DAG with a disappearing index $i$. \textit{Middle}: Modified DAG where the sum over index $i$ is made explicit. \textit{Right}: Derivative of the middle DAG with respect to $A$.}
